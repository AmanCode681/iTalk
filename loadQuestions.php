...
require "dbconnect.php";
        ...
        $remaining_ques=$total_rows-$nums_questions;
            if($remaining_ques>2)
            {
                $show_questions=2;
            }
            ...
            {
                $show_questions=$remaining_ques;
                $show=true;
            }
        
        $sql="select * from thread_ques where tcid=$id and deleted=0 order by dt desc limit $show_questions offset $nums_questions ";
        ...
        if($remaining_ques<=4)
        {
            $cid=$_GET['id'];
            echo'
            <button type="button" id="prevbtn" class="btn btn-outline-success" onclick="loadLessQuestions('.$cid.','.($nums_questions-2).')">prev</button>
            </div>
            </div>
        ';
        }
        else
        {
          $cid=$_GET['id'];
            ...
            <button type="button"  id="nextflex" class="btn btn-outline-success" onclick="loadMoreQuestions('.$cid.','.($show_questions+$nums_questions).')">next</button>
            </div>
            </div>
            </div>';
        }
